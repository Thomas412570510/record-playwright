...
def inspect_url(url, account="TestUser", password="WrongPassword123!"):
    print(f"\n🔍 [啟動資安傳輸稽核器] 目標網址: {url}")
    print(f"👤 測試帳號: {account}")
    print(f"🔑 測試密碼: {password}")
    ...
    with sync_playwright() as p:
        ...
        # 1. 綁定稽核器
        auditor = PayloadAuditor(page, input_account=account, input_password=password)

        try:
            page.goto(url, timeout=15000, wait_until="networkidle")
        except Exception:
            print("⚠️ 網頁載入緩慢或超時，繼續執行...")
    ...
    # 3. 輸出符合查核報表格式之結果
    report_text = auditor.get_audit_findings_text()
    print("\n================ [資安查核產出文字] ================")
    print(report_text)
    print("====================================================\n")
    return report_text
